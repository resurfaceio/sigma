...
# Copyright 2020 Jonas Hagg, Anish Shrestha
...
class ResurfaceBackend(SQLBackend):
    """Converts Sigma rule into SQL query for resurface"""

    identifier = "resurface"
    active = True
    ...
    def requireFTS(self, node):
        return not self.mappingItem and (
            type(node) in (int, str)
            or all(isinstance(val, str) for val in node)
            or all(isinstance(val, int) for val in node)
        )
    ...
    def generateANDNode(self, node):
        ...
        if self.requireFTS(node):
            fts = str('"' + self.andToken + '"').join(
                self.cleanValue(val) for val in node
            )
